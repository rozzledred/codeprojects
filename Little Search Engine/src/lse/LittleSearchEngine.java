...
public class LittleSearchEngine {
	...
	public HashMap<String, Occurrence> loadKeywordsFromDocument(String docFile) throws FileNotFoundException {
		...
		if (!file.exists()) {
			throw new FileNotFoundException("File " + docFile + " not found");
		}
	...
	public ArrayList<Integer> insertLastOccurrence(ArrayList<Occurrence> occs) {
		ArrayList<Integer> midpointarr = new ArrayList<>();
		...
		while (true) {
			...
			if (midocc.frequency == frequencyToInsert) {
				i = mid;
				break;
			} else if (midocc.frequency < frequencyToInsert) {
				hi = mid - 1;
				if (lo > hi) {
					i = mid;
					break;
	...
	public ArrayList<String> top5search(String kw1, String kw2) {
		...
		if ((kw1 == null && kw2 == null) || (!keywordsIndex.containsKey(kw1) && !keywordsIndex.containsKey(kw2))
			...
			occurence.addAll(keywordsIndex.get(kw1));
			occurence.addAll(keywordsIndex.get(kw2));
			for (int count = 0; count < 5 && !occurence.isEmpty(); count++) {
				int ptr = 0;
				int prev = -1;
				for (ptr = 0; ptr < occurence.size() && occurence.get(ptr) != null; ptr++) {
					if (prev == -1) {
						if (!result.contains(occurence.get(ptr).document))
							prev = ptr;
					} else if (occurence.get(ptr).frequency > occurence.get(prev).frequency) {
						if (!result.contains(occurence.get(ptr).document))
							prev = ptr;
					} else if (occurence.get(ptr).frequency == occurence.get(prev).frequency) {
						if (keywordsIndex.get(kw1).contains(occurence.get(ptr))) {
							if (!result.contains(occurence.get(ptr).document))
								prev = ptr;
						}
					}
				...
				if (prev != -1)
					result.add(occurence.remove(prev).document);
			}
			return result;
		}
	}
